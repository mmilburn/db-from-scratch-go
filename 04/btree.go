...
type BTree struct {
	//page pointer
	root uint64
	// callbacks for working with on-disk pages
...
func nodeReplaceKidN(tree *BTree, new BNode, old BNode, idx uint16, kids ...BNode) {
	...
	if inc == 1 && bytes.Equal(kids[0].getKey(0), old.getKey(idx)) {
		// common case, replace only 1 pointer
...
func nodeSplit2(left BNode, right BNode, old BNode) {
	...
	// the left half may still be too large
...
func nodeSplit3(old BNode) (uint16, [3]BNode) {
	...
	left := BNode(make([]byte, 2*BTREE_PAGE_SIZE)) // could be split later
...
// insert the KV into a node, the result might be split.
// the caller is responsible for deallocating the input mode.
